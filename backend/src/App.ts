...
import { ServeNextJsFrontEndAppService } from "./services/ServeNextJsFrontEndAppService.js";

class App {
  private app: Express;
  private port: number;
  private server: any;
  private nextJsService?: ServeNextJsFrontEndAppService;

  constructor() {
    this.app = express();
    this.port = Number(Env.PORT);
    Env.NODE_ENV === "production" &&
      (this.nextJsService = new ServeNextJsFrontEndAppService());
  }
  ...
  public async initialize(): Promise<void> {
    ...
    Env.NODE_ENV === "production" && (await this.initializeNextJs());
  }

  private async initializeNextJs(): Promise<void> {
    if (!this.nextJsService) return;
    await this.nextJsService.init();
    // Use Next.js for all non-API routes
    this.app.get(/^(?!\/api).*$/, this.nextJsService.getRequestHandler());
  }
    ...
    if (this.server) {
      ...
      return new Promise((resolve, reject) => {
        this.server.close(async (err: any) => {
          ...
          await this.nextJsService?.close();
          await db.disconnect();
  ...
  private initializeRoutes(): void {
    ...
    this.app.use(
      "/api/auth",
      createAuthRoutes(authService, clientMetadataService)
    );
